...
Reference 与角色标注独立：候选→生成参考文件→人工批准→可进入声音配置；停用不删除旧文件。每个 profile 首版固定一个已批准参考（角色库可有很多），保存真实部署配置和 seed 的快照，禁止从前端提交 Python/命令路径。先生成测试音，再由用户显式批准并设为默认（按 2026-09-14 用户要求，不收集审核人姓名，记录 local-user）。加入新参考不会自动改变 profile。
